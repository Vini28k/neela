/*
  ...
  1. New Tables
    ...
    - `heart_rate_data` - Heart rate measurements from devices
    - `emergency_contacts` - Emergency contact information
    - `ml_predictions` - Machine learning predictions for crisis detection
    - `user_baselines` - User-specific baseline measurements
...
SELECT create_policy_if_not_exists(
  'Users can read own weather states',
  'weather_states',
  'CREATE POLICY "Users can read own weather states" ON weather_states FOR SELECT TO authenticated USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Users can insert own weather states',
  'weather_states',
  'CREATE POLICY "Users can insert own weather states" ON weather_states FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Users can update own weather states',
  'weather_states',
  'CREATE POLICY "Users can update own weather states" ON weather_states FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Service role can manage all weather states',
  'weather_states',
  'CREATE POLICY "Service role can manage all weather states" ON weather_states FOR ALL TO service_role USING (true) WITH CHECK (true)'
);

-- Create RLS policies for heart_rate_data
SELECT create_policy_if_not_exists(
  'Users can read own heart rate data',
  'heart_rate_data',
  'CREATE POLICY "Users can read own heart rate data" ON heart_rate_data FOR SELECT TO authenticated USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Users can insert own heart rate data',
  'heart_rate_data',
  'CREATE POLICY "Users can insert own heart rate data" ON heart_rate_data FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Users can update own heart rate data',
  'heart_rate_data',
  'CREATE POLICY "Users can update own heart rate data" ON heart_rate_data FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Service role can manage all heart rate data',
  'heart_rate_data',
  'CREATE POLICY "Service role can manage all heart rate data" ON heart_rate_data FOR ALL TO service_role USING (true) WITH CHECK (true)'
);

-- Create RLS policies for emergency_contacts
SELECT create_policy_if_not_exists(
  'Users can read own emergency contacts',
  'emergency_contacts',
  'CREATE POLICY "Users can read own emergency contacts" ON emergency_contacts FOR SELECT TO authenticated USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Users can insert own emergency contacts',
  'emergency_contacts',
  'CREATE POLICY "Users can insert own emergency contacts" ON emergency_contacts FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Users can update own emergency contacts',
  'emergency_contacts',
  'CREATE POLICY "Users can update own emergency contacts" ON emergency_contacts FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Users can delete own emergency contacts',
  'emergency_contacts',
  'CREATE POLICY "Users can delete own emergency contacts" ON emergency_contacts FOR DELETE TO authenticated USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Service role can manage all emergency contacts',
  'emergency_contacts',
  'CREATE POLICY "Service role can manage all emergency contacts" ON emergency_contacts FOR ALL TO service_role USING (true) WITH CHECK (true)'
);

-- Create RLS policies for ml_predictions
SELECT create_policy_if_not_exists(
  'Users can read own ML predictions',
  'ml_predictions',
  'CREATE POLICY "Users can read own ML predictions" ON ml_predictions FOR SELECT TO authenticated USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Service role can manage ML predictions',
  'ml_predictions',
  'CREATE POLICY "Service role can manage ML predictions" ON ml_predictions FOR ALL TO service_role USING (true) WITH CHECK (true)'
);

-- Create RLS policies for user_baselines
SELECT create_policy_if_not_exists(
  'Users can read own baselines',
  'user_baselines',
  'CREATE POLICY "Users can read own baselines" ON user_baselines FOR SELECT TO authenticated USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Service role can manage baselines',
  'user_baselines',
  'CREATE POLICY "Service role can manage baselines" ON user_baselines FOR ALL TO service_role USING (true) WITH CHECK (true)'
);
...
SELECT create_policy_if_not_exists(
  ...
  'notification_logs',
  'CREATE POLICY "Users can read own notifications" ON notification_logs FOR SELECT TO authenticated USING (auth.uid() = user_id)'
);

SELECT create_policy_if_not_exists(
  'Service role can manage notifications',
  'notification_logs',
  'CREATE POLICY "Service role can manage notifications" ON notification_logs FOR ALL TO service_role USING (true) WITH CHECK (true)'
);
