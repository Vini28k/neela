/*
  # Mental Weather Database Schema Migration
  
  1. New Tables
    - `profiles` - User profile information linked to auth.users
    - `weather_states` - User emotional weather tracking
...
DROP POLICY IF EXISTS "Users can read own weather states" ON weather_states;
DROP POLICY IF EXISTS "Users can insert own weather states" ON weather_states;
DROP POLICY IF EXISTS "Users can update own weather states" ON weather_states;
DROP POLICY IF EXISTS "Service role can manage all weather states" ON weather_states;

DROP POLICY IF EXISTS "Users can read own heart rate data" ON heart_rate_data;
...
DROP POLICY IF EXISTS "Service role can manage all heart rate data" ON heart_rate_data;
...
DROP POLICY IF EXISTS "Service role can manage all emergency contacts" ON emergency_contacts;
